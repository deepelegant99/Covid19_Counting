...
import DisplayData from "../components/DisplayItems";
...
const ContinentPage = ({ data }) => {
  ...
  /****************************************** */
  const europeList = europeCountries.map(
    (europeCountries) => data[europeCountries]["All"]
  );

  let europeLife_expectancy = 0;

  const europeResult3 = Object.keys(europeList).map(function (countrylife) {
    return (europeLife_expectancy =
      europeLife_expectancy +
      parseFloat(europeList[countrylife]["life_expectancy"]));
  });
  /****************************************** */
  const EuropeCountries = Object.keys(data).filter(function (countryName) {
  ...
  const naDeaths = naCountries.map((country) => data[country]["All"]["deaths"]);
  ...
  const saDeaths = saCountries.map((country) => data[country]["All"]["deaths"]);
  ...
  const OceaniaCountries = Object.keys(data).filter(function (countryName) {
    ...
    console.log(
      "Deaths: " +
        ...
        " " +
        "Confirmed: " +
        oceaniaConfirmedCount +
        " Life Expectancy: " +
        "australiaLife_expectancy"
    );
  });
